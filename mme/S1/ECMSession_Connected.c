...
static void processMsg(gpointer _ecm, S1AP_Message_t *s1msg, int r_sid){
    ...
    if(s1msg->pdu->procedureCode == id_HandoverPreparation &&
       s1msg->choice == initiating_message){
        ecm_log(ecm, LOG_WARNING, 0, "Received id_HandoverPreparation");
    }else if(s1msg->pdu->procedureCode ==  id_HandoverCancel &&
             s1msg->choice == initiating_message){
        ecm_log(ecm, LOG_WARNING, 0, "Received id_HandoverCancel");
    }else if(s1msg->pdu->procedureCode ==  id_UEContextRelease &&
             s1msg->choice == successful_outcome){
        ecm_log(ecm, LOG_DEBUG, 0, "Received id_UEContextReleaseComplete");
        if(ecm->causeRelease){
            c = ecm->causeRelease;
        ...
        if (eNB_ID->eNB_id != ecm->eNBUEId || mme_id->mme_id != ecm->mmeUEId){
            ecm_log(ecm, LOG_WARNING, 0, "Received InitialContextSetupResponse"
                    " with incorrect IDs");
            return;
        }
        ecm_log(ecm, LOG_DEBUG, 0, "Received InitialContextSetupResponse");
        ...
        ecm_log(ecm, LOG_WARNING, 0, "Received InitialContextSetupFailure");
    ...
    /* Class 2 Procedures*/
        ...
        eNB_ID = (ENB_UE_S1AP_ID_t*)s1ap_findIe(s1msg, id_eNB_UE_S1AP_ID);
        if (eNB_ID->eNB_id != ecm->eNBUEId || mme_id->mme_id != ecm->mmeUEId){
            ecm_log(ecm, LOG_WARNING, 0, "Received id_uplinkNASTransport with incorrect IDs");
            return;
        }
        ...
        emm_processMsg(ecm->emm, nASPDU->str, nASPDU->len);
             ...
             s1msg->choice == initiating_message){
        ecm_log(ecm, LOG_WARNING, 0, "Received id_HandoverNotification");
    }else if(s1msg->pdu->procedureCode == id_NASNonDeliveryIndication &&
             s1msg->choice == initiating_message){
        ecm_log(ecm, LOG_WARNING, 0, "Received id_NASNonDeliveryIndication");
    }else if(s1msg->pdu->procedureCode == id_UEContextReleaseRequest &&
             s1msg->choice == initiating_message){
        ecm_log(ecm, LOG_DEBUG, 0, "Received id_UEContextReleaseRequest");
        ecm_UEContextRelease(ecm, s1msg);
    }else if(s1msg->pdu->procedureCode == id_UECapabilityInfoIndication &&
             s1msg->choice == initiating_message){
        ecm_log(ecm, LOG_DEBUG, 0, "Received id_UECapabilityInfoIndication");
    }else if(s1msg->pdu->procedureCode == id_eNBStatusTransfer &&
             s1msg->choice == initiating_message){
        ecm_log(ecm, LOG_WARNING, 0, "Received id_eNBStatusTransfer");
    }else if(s1msg->pdu->procedureCode == id_TraceFailureIndication &&
             s1msg->choice == initiating_message){
        ecm_log(ecm, LOG_WARNING, 0, "Received id_TraceFailureIndication");
    }else if(s1msg->pdu->procedureCode == id_LocationReportingFailureIndication &&
             s1msg->choice == initiating_message){
        ecm_log(ecm, LOG_WARNING, 0, "Received id_LocationReportingFailureIndication");
    }else if(s1msg->pdu->procedureCode == id_LocationReport &&
             s1msg->choice == initiating_message){
        ecm_log(ecm, LOG_WARNING, 0, "Received id_LocationReport");
    }else if(s1msg->pdu->procedureCode == id_CellTrafficTrace &&
             s1msg->choice == initiating_message){
        ecm_log(ecm, LOG_WARNING, 0, "Received id_CellTrafficTrace");
    }else{
        ecm_log(ecm, LOG_ERR, 0, "Message not recognized in this context");
    }
}
