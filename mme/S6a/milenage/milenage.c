...
/**
 * get OPC - Derive OPc from OP and K
 * @op: OPc = 128-bit value OP
...
void getOPC(const uint8_t *op, const uint8_t *k, uint8_t *opc){
    ...
    aes_128_encrypt_block(k, op, tmp1);
 ...
 */
static void milenage_f1(const uint8_t *opc, const uint8_t *k, const uint8_t *_rand,
 ...
 */
static void milenage_f2345(const uint8_t *opc, const uint8_t *k, const uint8_t *_rand,
 ...
 */
static void milenage_opc(const uint8_t *op, const uint8_t *k, uint8_t *opc)
{
	int i;
	/* OP_C = OP XOR E_K(OP) */
...
{
	uint8_t buf[16], buf2[16], buf3[16], buf4[16], buf5[16], opc[16];
	uint8_t auts[14], sqn[6], _rand[16];
	int ret = 0, res, i;
